...
INDICATORS = {
    ...
    # durations removed in new layout
...
# Header
st.title("From Industry to Innovation: The Transformation of China's Society (2000–2020)")
st.markdown(
    """
    China's transformation from traditional industrial economy to innovation-driven knowledge society. 
    Explore R&D investment, education expansion, digitalization, and social modernization.
    """
)
...
# Groupings for new sections
digital_codes = [
    INDICATORS["INTERNET_USERS_PCT"],
    INDICATORS["MOBILE_SUB_PER100"],
    INDICATORS["BROADBAND_SUBSCRIBERS"],
    INDICATORS["ELEC_USE_PER_CAP"],
]
knowledge_codes = [
    INDICATORS["RND_PCT_GDP"],
    INDICATORS["RESEARCHERS_PER_MILL"],
    INDICATORS["PATENTS_RESIDENTS"],
    INDICATORS["SCIENTIFIC_ARTICLES"],
]
governance_codes = [
    INDICATORS["GENDER_PARLIAMENT"],
    INDICATORS["WOMEN_BUSINESS_LAW"],
    INDICATORS["GOV_EFFECTIVENESS"],
    INDICATORS["RULE_OF_LAW"],
    INDICATORS["CONTROL_CORRUPTION"],
    INDICATORS["REGULATORY_QUALITY"],
]
talent_codes = [
    INDICATORS["TERTIARY_ENROLLMENT"],
    INDICATORS["RESEARCHERS_PER_MILL"],
    INDICATORS["NET_MIGRATION"],
]
...
internet_df = data[INDICATORS["INTERNET_USERS_PCT"]]
mobile_df = data[INDICATORS["MOBILE_SUB_PER100"]]
broadband_df = data[INDICATORS["BROADBAND_SUBSCRIBERS"]]
elec_df = data[INDICATORS["ELEC_USE_PER_CAP"]]

# 1) S-curve: Internet users (% population)
...
# 2) Dual line: Mobile vs Broadband
...
rnd_df = data[INDICATORS["RND_PCT_GDP"]]
res_df = data[INDICATORS["RESEARCHERS_PER_MILL"]]
pat_df = data[INDICATORS["PATENTS_RESIDENTS"]]

# Line: R&D % GDP + Researchers (dual axis)
...
# Patent counts: small timeline bar
if not pat_df.empty:
...
#########################################################
# SECTION C: Governance Modernization & Inclusion         #
#########################################################
st.markdown("---")
st.subheader("🏙️ Social Modernization & Human Development")

ter_df = data[INDICATORS["TERTIARY_ENROLLMENT"]]
women_parl_df = data[INDICATORS["GENDER_PARLIAMENT"]]

# Line: Tertiary enrollment
if not ter_df.empty:
...
# Women in parliament: time series with latest annotation
if not women_parl_df.empty:
    wp = decade_filter(women_parl_df.copy(), decade)
    fig_s2 = go.Figure()
    ...
    ly = int(wp['Year'].max())
    lv = float(wp[wp['Year'] == ly]['Value'].iloc[0])
...
st.subheader("🎯 Innovation & Society – Conclusion")
st.markdown(
    "> China’s society is becoming more digital, research-driven, and educated—R&D and talent deepen while connectivity expands, with steady gains in inclusion."
)
