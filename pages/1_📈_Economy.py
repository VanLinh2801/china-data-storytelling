...
INDICATORS = {
    ...
    # Trade & Finance (merged from Trade & Finance page)
...
# ---------------------------
# Chart 1 – Topline Dynamics (Two charts side by side)
# ---------------------------
...
# ---------------------------
# Chart 5 – GDP Composition Treemap
# ---------------------------
st.subheader("Economic Composition Snapshot")
# Make treemap full-width
comp = (
...
# Chart 6 removed per request

# ---------------------------
# GDP Sectors & Trade Openness (side by side)
# ---------------------------
...
# ---------------------------
# Additional Trade & Finance charts (you can re-order later)
# ---------------------------
...
try:
    ...
    if trade_regions_path.exists():
        ...
        if expected_cols.issubset(set(tri.columns)):
            ...
            # Two fixed years side by side (2000 vs 2020)
            ...
            # Render vertically: 2000 on top, 2020 below
            for y_sel in [2000, 2020]:
                ...
                if tri_y.empty:
                    ...
                    china_pos = (0.0, 0.0)
                    # Three-ring layout based on value quantiles (reduces overlap)
                    ...
                    for rname, v in zip(regions, tri_sorted[value_col].values):
                        if v <= q1:
                            ...
                            outer.append(rname)
                    region_positions = {}
                    # place with jitter to avoid spokes alignment
                    ...
                    for _, row in tri_sorted.iterrows():
                        ...
                        fig_network.add_trace(go.Scatter(x=[x], y=[y], mode="markers",
                                                         marker=dict(size=size + 8, color="rgba(0,0,0,0.08)", line=dict(width=0)),
                                                         hoverinfo="skip", showlegend=False))
                        # always show text; abbreviate to T if >= 1000B
                        ...
                        fig_network.add_trace(go.Scatter(x=[x], y=[y], mode="markers+text",
                                                         marker=dict(size=size, color=color, line=dict(width=4, color="white"), opacity=0.96),
                                                         text=[display_text], textposition="middle center",
                                                         textfont=dict(color="white", size=font_size, family="Inter, Arial"),
                                                         hovertemplate=f"<b>{region}</b><br>{title_suffix}: ${vb:.1f}B USD<extra></extra>",
                                                         showlegend=False))
                    # add legend swatches (for both 2000 and 2020)
                    for rname in regions:
                        fig_network.add_trace(
                            go.Scatter(x=[None], y=[None], mode="markers",
                                       marker=dict(size=12, color=region_colors.get(rname, "#6c757d")),
                                       name=rname, showlegend=True)
                        )
                    # China node (keep text short inside; put label outside)
                    total_b = tri_sorted[value_col].sum() / 1e9
                    fig_network.add_trace(go.Scatter(x=[china_pos[0]], y=[china_pos[1]], mode="markers+text",
                                                     marker=dict(size=56, color="#d33f49", line=dict(width=6, color="white"), opacity=0.98),
                                                     text=[(f"${total_b/1000:.1f}T" if total_b >= 1000 else f"${total_b:.1f}B")], textposition="middle center",
                                                     textfont=dict(color="white", size=10, family="Inter, Arial Black"),
                                                     hovertemplate=f"<b>China</b><br>Total {title_suffix.lower()}: ${total_b:.1f}B USD<extra></extra>",
                                                     showlegend=False))
                    fig_network.add_annotation(x=china_pos[0], y=china_pos[1]-0.6, text="China", showarrow=False,
                                               font=dict(size=12, color="#2f2f2f"), xanchor="center", yanchor="top")
                    # layout
                    fig_network.update_layout(
                        ...
                        shapes=[
                            dict(type="circle", xref="x", yref="y", x0=-1.15, y0=-1.15, x1=1.15, y1=1.15,
                                 line=dict(color="rgba(0,0,0,0.06)", width=1), layer="below"),
                            dict(type="circle", xref="x", yref="y", x0=-1.85, y0=-1.85, x1=1.85, y1=1.85,
                                 line=dict(color="rgba(0,0,0,0.04)", width=1), layer="below"),
                        ]
                    )
