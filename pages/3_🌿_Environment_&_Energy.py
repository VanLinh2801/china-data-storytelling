...
# Streamlit UI
# ---------------------------
st.set_page_config(page_title="China's Environmental Transformation (2000–2020)", layout="wide", page_icon="🌿")
...
st.markdown(
    ...
    <style>
      ...
      .chart-description {
        background: #f1f3f4; padding: 1rem; margin: 0.5rem 0; 
...
# Sidebar
st.sidebar.header("🌿 Environmental Story Filters")
decade = st.sidebar.selectbox("Time Period", ["All", "2000s", "2010s"], index=0, key="env_decade")

# Header
st.title("🌿 China's Environmental Transformation (2000–2020)")
...
if not ghg_data['CO2_TOTAL'].empty and not ghg_data['CH4_TOTAL'].empty and not ghg_data['N2O_TOTAL'].empty:
    st.subheader("🌫️ Greenhouse Gas Composition Over Time")
...
# ---------------------------
# STORY CONCLUSION: The Path Forward
# ---------------------------
st.markdown("---")
st.subheader("🎯 Environmental Performance Summary")

# Final Summary Chart: Environmental Performance Index
if not ghg_data['CO2_PER_CAPITA'].empty and not pollution_data['PM25'].empty and not energy_data['ELEC_RENEW'].empty:
    
    # Create normalized environmental performance index
    co2_pc = ghg_data['CO2_PER_CAPITA'].copy()
    pm25 = pollution_data['PM25'].copy()
    renew = energy_data['ELEC_RENEW'].copy()
    
    # Merge data
    summary_data = co2_pc.merge(pm25, on='Year', how='outer').merge(renew, on='Year', how='outer')
    summary_data = decade_filter(summary_data, decade)
    summary_data = summary_data.ffill().bfill()
    
    # Normalize indicators (invert CO2 and PM2.5 for better visualization)
    summary_data['CO2_norm'] = 100 - minmax_normalize(summary_data['Value_x'])  # Inverted
    summary_data['PM25_norm'] = 100 - minmax_normalize(summary_data['Value_y'])  # Inverted
    summary_data['Renew_norm'] = minmax_normalize(summary_data['Value'])  # Direct
    
    # Calculate composite index
    summary_data['Environmental_Index'] = (summary_data['CO2_norm'] + summary_data['PM25_norm'] + summary_data['Renew_norm']) / 3
    
    fig15 = go.Figure()
    fig15.add_trace(go.Scatter(
        x=summary_data['Year'], y=summary_data['Environmental_Index'],
        mode='lines+markers', name='Environmental Performance Index',
        line=dict(color='#2ca25f', width=4),
        marker=dict(size=10)
    ))
    
    fig15.update_layout(
        title="China's Environmental Performance Index (0-100)",
        xaxis_title="Year", yaxis_title="Performance Index",
        height=500, margin=dict(t=60, b=50, l=50, r=30)
    )
    st.plotly_chart(fig15, use_container_width=True)

# Concise conclusion
st.markdown(
    "> China is bending the curve: emissions intensity falls as renewables rise, air quality improves, and forests recover—signaling steady progress toward a cleaner growth model."
)
...
with col_right:
    if st.button("Next: Trade & Finance →", use_container_width=True):
        st.switch_page("pages/4_💹_Trade_&_Finance.py")